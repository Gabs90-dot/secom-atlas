...
function normalizeEntityName(value: string) {
  return String(value || "")
    ...
    .replace(/\bCOMP\.?\s*CC\b/g, "COMPAGNIA CARABINIERI")
    .replace(/\bCOM\.?\s*PROV\.?\s*CC\b/g, "COMANDO PROVINCIALE CARABINIERI")
...
function normalizeCompleteName(value: string) {
  const parts = splitPath(value);

  return parts.map(normalizeEntityName).join(" > ");
}
...
export async function syncGlpiEntitiesToAtlas({
  tenantId,
}: SyncGlpiEntitiesArgs) {
  ...
  try {
    ...
    for (const entity of entities) {
      ...
      try {
        ...
        const normalizedCompleteName =
          normalizeCompleteName(completeName);

        const parts = splitPath(normalizedCompleteName);
        ...
        const payload = {
          ...
          parent_glpi_entity_id: entity.entities_id
            ? Number(entity.entities_id)
            : null,
          ...
          normalized_complete_name: normalizedCompleteName,

          level,

          path_parts: parts,

          entity_type: detectEntityType(
            entity.name || "",
            normalizedCompleteName,
          ),
